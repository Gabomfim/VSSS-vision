...
Modelo leve & Usa apenas a combinação escolhida pelos testes comparativos, chamados de
...
\conceptbox{green}{
  ...
  uma \textbf{pseudo-etiqueta}, não uma verdade absoluta.
...
\conceptbox{blue}{
  ...
  quadro, o professor estima o centro em $(620,410)$ e o aluno em $(622,409)$. A distância
...
cálculos com números decimais e pode ser processada rapidamente tanto pelo professor quanto
pelo aluno; o professor apenas combina essa informação com mais evidências.
...
O detector leve principal usa uma \textbf{convolução} com um \textbf{núcleo} circular
pré-calculado. O núcleo é uma pequena tabela de números que funciona como um molde. A
...
rejeitada. A posição aceita recebe o nome de \textbf{pseudo-etiqueta}: é um rótulo produzido
automaticamente, útil para treinamento, mas ainda sujeito a erro.

Como trabalha fora de tempo real, o professor analisa o vídeo nos dois sentidos. Detecções
próximas feitas com passado e futuro são combinadas conforme a confiança; grandes diferenças
são rejeitadas. Um suavizador de Kalman bidirecional Rauch--Tung--Striebel ajusta posição e
...
\midrule
Professor & Quadro atual, passado e futuro; suavização bidirecional RTS. & Criar e conferir
pseudo-etiquetas em vídeo gravado.\\
Aluno & Quadro atual e estado calculado no passado; filtro de Kalman causal. & Entregar a
posição ao vivo durante o jogo.\\
\bottomrule
\end{tabularx}
\end{center}

Usar o futuro no professor não é vazamento para o sistema ao vivo: essa informação serve
somente para criar rótulos fora de tempo real. O aluno nunca recebe quadros futuros durante
...
\subsection{Ajuste do professor}
...
O melhor professor produz pseudo-etiquetas bidirecionais.

\subsection{Escolha do aluno por ablações}

O aluno permanece causal: usa
...
desligada. Uma \textbf{ablação} é esse teste controlado: muda-se uma parte do pipeline e
...
experimento. O erro de centro normalizado é a distância entre os centros do aluno e do
professor dividida pelo raio esperado da bola. Assim, no exemplo inicial, 2,2 pixels viram
...
previsão. Isso define a ROI com custo muito pequeno. É diferente do suavizador RTS do
professor, que revisa a trajetória depois de conhecer também os quadros futuros.
...
\textbf{vazão}, não garantem que o resultado mais recente chegou cedo. Por isso a captura usa
...
\begin{itemize}
  \item \textbf{O aluno imita o professor?} Compare seus centros e falhas no mesmo vídeo.
  \item \textbf{Os dois estão corretos?} Compare ambos com a amostra manual independente.
  \item \textbf{O aluno atende ao tempo real?} Meça a latência no computador e na câmera
  usados no jogo.
\end{itemize}
