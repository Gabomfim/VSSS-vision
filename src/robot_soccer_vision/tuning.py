...
def load_video_frames(path: str, maximum_frames: int = 600, stride: int = 1) -> list[np.ndarray]:
...
def tune_pipeline(
    ...
    frames = load_video_frames(video_path, maximum_frames)
    teacher_frames = frames[:: max(1, len(frames) // 90)]
    teacher, history = tune_teacher(
        teacher_frames, initial_center, initial_radius, teacher_trials
    )
    labels = ExpensiveTeacher(teacher).label(frames)
    ablations = run_student_ablation(frames, labels, teacher)
    save_report(
        output_directory,
        teacher,
        history,
